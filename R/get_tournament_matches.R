...
#'
#' @examples
#' ## Return match data for 2019's Tournament of Champions.
#' get_tournament_matches("tournament of champions", year = 2019)
#'
#' ## Return match data for all PSA World Tour tournaments in 2019
#' get_tournament_matches(year = 2019)
#'
#' ## Return match data for ALL tournaments in 2020
#' get_tournament_matches(year = 2020, world_tour = FALSE)
#'
#'
...
get_tournament_matches <- function(tournament = NULL, year = 2020, world_tour = TRUE) {
    ...
    for (i in 1:length(tournaments$slug)) {
      ...
      matches <- bind_rows(matches, match_result)

    }

    matches <- matches %>%
                  arrange(tournament_date, desc(round)) %>%
                  mutate(games = str_replace_all(games, pattern = regex(" \\([:digit:]{2,}m\\)"), replacement = ""),
