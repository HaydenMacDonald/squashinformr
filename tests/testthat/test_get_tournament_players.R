...
test_that("test get_tournament_players for proper outputs", {
  ...
  df <- squashinformr::get_tournament_players("CIB Egyptian Open", year = 2021, world_tour = TRUE)
  ...
  df <- squashinformr::get_tournament_players("CIB Egyptian Open", year = NULL, world_tour = TRUE)
  expect_is(df, "data.frame")
  expect_is(df, "tbl")
  expect_is(sample(df$tournament_date, 1), "Date")
  expect_equal(year(sample(df$tournament_date, 1)), 2021)
