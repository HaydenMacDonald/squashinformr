...
test_that("test get_tournament_players for wrong input errors", {
  ...
  expect_error(get_tournament_players("Tournament of Champions", year = "2020-01-01", world_tour = TRUE))
  expect_error(get_tournament_players("Tournament of Champions", year = 2020, world_tour = "TRUE"))
  expect_error(get_tournament_players("Premier League", year = 2020, world_tour = TRUE))
  expect_error(get_tournament_players("Tournament of Champions", year = 2018, world_tour = TRUE))
...
test_that("test get_tournament_players for proper outputs", {
  ...
  df <- get_tournament_players("Tournament of Champions", year = 2020, world_tour = TRUE)
  ...
  df <- get_tournament_players("Tournament of Champions", year = NULL, world_tour = TRUE)
  expect_is(df, "data.frame")
  expect_is(df, "tbl")
  expect_equal(length(unique(df$category)), 2)
  expect_is(sample(df$tournament_date, 1), "Date")
  ...
  df <- get_tournament_players(tournament = NULL, year = 2020, world_tour = TRUE)
  expect_is(df, "data.frame")
  expect_is(df, "tbl")
  expect_equal(length(unique(df$category)), 2)
  expect_is(sample(df$tournament_date, 1), "Date")
